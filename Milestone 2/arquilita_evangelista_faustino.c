...
Milestone 2:
Backtracking with Initial Board
***/
...
int main() {
    ...
    for (n = 0; n < num_boards; n++) {
        ...
        for (i = 1; i <= boardsize; i++) {
            ...
            for (token = strtok_r(buffer, " ", &rest); token != NULL; token = strtok_r(NULL, " ", &rest)) {
                ...
                if (atoi(token) != 0) {
                    ...
                    // check col validity
                    for (j = 1; j < i; j++) {
                        if (chancellor_position[j] == chancellor_position[i]) invalid_board = TRUE;
                    }
                }
                col++;
            }
        ...
        if (invalid_board == TRUE) {
            // more than one chancellors in one row or column
            fprintf(fout, ">> No solution found due to invalid board.\n\n");
